...
if __name__ == '__main__':
    config = DQNConfig()
    config.training(num_atoms=tune.grid_search(list(range(1,3))))
    
    from ParallelMultiSnapshotEnv2DAirfoil import ParallelMultiSnapshotEnv2DAirfoil as Env2DAirfoil
    
    from ray.tune.registry import register_env
    def env_creator(env_config):
        return Env2DAirfoil.remote(env_config)
    register_env("env_2d_airfoil", env_creator)
    
    config.environment(env="env_2d_airfoil")
    #config.td_error_loss_fn = "huber"
    tune.Tuner(
        "DQN",
        run_config=air.RunConfig(local_dir="./results", name="test_experiment", stop={"episode_reward_mean":200}),
        param_space=config.to_dict(),
    ).fit()
